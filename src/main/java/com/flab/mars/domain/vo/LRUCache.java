...
public class LRUCache<T> {
    ...
    private final Map<String, T> map = new HashMap<>(); // LinkedList 를 경우 키의 검색의 경우 O(n) 를 보완하기 위해서 map 생성-> 키의 유무 판단 O(1)
    ...
    public static <T> LRUCache<T> createWithDefaultCapacity() {
        return new LRUCache<>();
    }

    public static <T> LRUCache<T> createWithCapacity(int capacity) {
        if (capacity <= 0) {
